    ...
    def validate(self, attrs):
        ...
        try:
            if question_type == "one" or question_type == "text":
                obj = Answer.objects.get(question=attrs['question'].id, survey=attrs['survey'], user_id=attrs['user_id'])
...
class QuestionSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if question_type == 'one' or question_type == 'multiple' or question_type=='text':
            return attrs
        raise serializers.ValidationError('Question type error')
...
class SurveySerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        if 'pub_date' in validated_data:
            raise serializers.ValidationError({'pub_date': 'this field can"t be changed.'})
        for key, value in validated_data.items():
            setattr(instance, key, value)
        instance.save()
        return instance
